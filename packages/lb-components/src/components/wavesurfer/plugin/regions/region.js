...
export class Region {
    constructor(params, regionsUtils, ws) {
        ...
        this.showTooltip = params.showTooltip ?? false;
    ...
    play(start) {
        ...
        this.wavesurfer.play(s, this.loop ? undefined : this.end);
        this.fireEvent('play');
    ...
    updateRender() {
        ...
        if (this.element != null) {
            ...
            this.style(this.element, {
                ...
                cursor: this.drag && this.canMove ? 'move' : 'inherit',
                zIndex: this.selected ? 4 : 3,
            });
    ...
    bindEvents() {
        ...
        this.element.addEventListener('click', (e) => {
            ...
            this.wavesurfer.fireEvent(
                ...
                this,
                e,
                this.regionsUtil.getRegionSnapToGridValue(
                    this.wavesurfer.drawer.handleEvent(e) * this.wavesurfer.getDuration()
                )
            );
        });
    ...
    bindDragEvents() {
        ...
        const onDown = (event) => {
            ...
            this.wavesurfer.fireEvent("regions-eventDown", {
                action: "resize",
                id: this.id,
                eventDownTime: startTime,
            });
        ...
        const onMove = (event) => {
            ...
            this.wavesurfer.fireEvent("region-mousemove", this, event);
            if (event.touches && event.touches.length > 1) {
                return;
            }
            ...
            if (
                this.wavesurfer.range?.min &&
                time < this.wavesurfer.range?.min
            ) {
                time = this.wavesurfer.range?.min;
            }
            if (
                this.wavesurfer.range?.max &&
                time > this.wavesurfer.range?.max
            ) {
                time = this.wavesurfer.range?.max;
            }
